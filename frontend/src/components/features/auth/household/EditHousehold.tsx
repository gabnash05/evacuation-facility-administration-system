...
export function EditHouseholdModal({ householdId, isOpen, onClose, onSuccess }: EditHouseholdModalProps) {
        ...
        if (isOpen && householdId) {
            const fetchData = async () => {
                ...
                try {
                    ...
                    const formattedIndividuals = individualsResult.data.map((ind: Individual) => ({ ...ind, date_of_birth: ind.date_of_birth ? ind.date_of_birth.split('T')[0] : '', }));
    ...
    const handleIndividualChange = (index: number, field: keyof Individual, value: string) => {
        if (field === 'relationship_to_head' && value.toLowerCase().trim() === 'head') {
            const otherHeadExists = individuals.some((ind, i) => i !== index && ind.relationship_to_head.toLowerCase().trim() === 'head');
            if (otherHeadExists) { alert("Error: A household head already exists."); return; }
        }
                        ...
                        <div className="space-y-4 py-4 max-h-[70vh] overflow-y-auto pr-6">
                            ...
                            <div className="border-t pt-4 space-y-2">
                                ...
                                <div className="space-y-2">
                                    <div className="grid grid-cols-[1fr_1fr_0.75fr_0.75fr_1fr_auto] gap-2 items-center text-sm font-medium text-muted-foreground px-2"><span>First Name</span><span>Last Name</span><span>D.O.B</span><span>Gender</span><span>Relationship</span><span></span></div>
                                    {individuals.map((ind, index) => (
                                        <div key={ind.individual_id || `new-${index}`} className="grid grid-cols-[1fr_1fr_0.75fr_0.75fr_1fr_auto] gap-2 items-center">
                                            <Input value={ind.first_name} onChange={(e) => handleIndividualChange(index, 'first_name', e.target.value)} />
                                            <Input value={ind.last_name} onChange={(e) => handleIndividualChange(index, 'last_name', e.target.value)} />
                                            <Input type="date" value={ind.date_of_birth || ''} onChange={(e) => handleIndividualChange(index, 'date_of_birth', e.target.value)} />
                                            <Select value={ind.gender || ''} onValueChange={(value) => handleIndividualChange(index, 'gender', value)}><SelectTrigger><SelectValue placeholder="-" /></SelectTrigger><SelectContent><SelectItem value="Male">Male</SelectItem><SelectItem value="Female">Female</SelectItem><SelectItem value="Other">Other</SelectItem></SelectContent></Select>
                                            <Input value={ind.relationship_to_head} onChange={(e) => handleIndividualChange(index, 'relationship_to_head', e.target.value)} />
