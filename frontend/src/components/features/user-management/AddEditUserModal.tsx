...
const getFormSchema = (isEditMode: boolean) =>
    (isEditMode ? updateUserSchema : createUserSchema).refine(
...
export function AddEditUserModal({
    ...
    const isCenterFieldLocked = isCenterAdmin && userCenterId;
        ...
        if (isOpen) {
            if (isEditMode && userToEdit) {
                ...
                const defaultCenterId =
                    isCenterAdmin && userCenterId ? String(userCenterId) : undefined;
                ...
                reset({
                    ...
                    role:
                        availableRoles.length > 0
                            ? (availableRoles[0].value as UserFormData["role"])
                            : "volunteer",
    ...
    const onSubmit = async (data: UserFormData) => {
        ...
        if (isCenterAdmin && userCenterId) {
            ...
            if (data.center_id && data.center_id !== "") {
                ...
                payload.center_id = Number.isFinite(n) ? n : undefined;
            } else {
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {isEditMode
                            ? `Update the details for ${userToEdit?.email}.`
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 py-4">
                    {submitError && (
                        <div
                            className="bg-destructive/10 text-destructive p-2 rounded"
                            role="alert"
                        >
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                            ...
                            <div className="col-span-3">
                                {isCenterFieldLocked ? (
                                    ...
                                    <div
                                        className={cn(
                                            "flex items-center h-10 px-3 py-2 text-sm",
                                            "border border-input rounded-md bg-muted"
                                        )}
                                    >
                                        {centers.find(center => center.center_id === userCenterId)
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            ...
                                            >
                                                <SelectTrigger aria-label="User center">
