...
def get_centers() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        logger.info("Fetching evacuation centers with filters: search=%s, status=%s", search, status)
        
        # Get centers with pagination and filtering
        centers_data = get_all_centers(search=search, status=status, page=page, limit=limit)
