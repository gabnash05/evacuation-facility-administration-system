...
class EvacuationCenter(db.Model):
    ...
    def get_all_active(cls) -> List['EvacuationCenter']:
        ...
        results = db.session.execute(
            text("SELECT * FROM evacuation_center WHERE status = 'active' ORDER BY center_id")
        ).fetchall()
    ...
    @classmethod
    def create_center(cls, address: str, capacity: int, status: str = 'active') -> 'EvacuationCenter':
